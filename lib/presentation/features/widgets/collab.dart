...
class CollabPage extends StatefulWidget {
  ...
  const CollabPage({required this.userName, required this.userFollowers, required this.accessToken, super.key});
...
class CollabPageState extends State<CollabPage> {
  ...
  TextEditingController userInput = TextEditingController(); ///handles input from user, used to search up songs
  TextEditingController userPlaylistName = TextEditingController();//names collab playlist
  ...
  Stream<DocumentSnapshot>? sessionStream; //meant to constantly communicate with database and establish a session
  ...
  child: activeSessionId != null
      ? StreamBuilder<DocumentSnapshot>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {//if user is waiting for someone to join, user is prompted with a display with a loading circle
            ...
            if (snapshot.hasData && snapshot.data != null) {//if session is able to be created, display info about users in session
              var sessionData = snapshot.data!.data() as Map<String, dynamic>;
              List users = sessionData['users'];
              List songs = sessionData['songs'];

              return Column( //display info on the current session
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Session ID: $activeSessionId', style: TextStyle(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16.0),
                  Text('Users in Session:', style: TextStyle(fontWeight: FontWeight.bold)),
                  ...users.map((user) => Text(user)),
                  const SizedBox(height: 16.0),
                  Text('Playlist:', style: TextStyle(fontWeight: FontWeight.bold)),
                  Expanded(
          ...
          : Column(//user is able to search for songs
              children: [
                // Search field
                TextField(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              trailing: ElevatedButton(
                                onPressed: () {//able to add songs to database once session established
                                  if (activeSessionId != null) {
                                    addSongToSession(activeSessionId!, track, context);
                                  } else {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(content: Text('You must create or join a session to add songs')),
                                    );
                                  }
                                },
  ...
  void promptForPlaylistName() { //allows user to enter a playlist name
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {//once clicked, collab playlist with given name is created
 ...
 Future<void> createCollaborativePlaylist(String collabName) async {//collab playlist added to list of user's playlists
...
Future<void> searchSongs(String query) async { //enables users to search for songs using Spotify's API
  ...
  Future<void> addToCollaborativePlaylist(String playlistId, String trackUri) async { //meant to add song to given collab playlist
    final response = await http.post(
  ...
  Future<void> fetchUserInfo() async { //get user data
    final response = await http.get(
  ...
  Future<void> fetchUserPlaylists() async { //get user's playlists to be displayed
  ...
  void showPlaylistTracksDialog(String playlistName) {//show the contents of playlist
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
  ...
  Future<void> openSpotifyTrack(String trackUri) async {
        final trackId = trackUri.split(':').last;  //extracts trackid from given spotify url
  ...
  Future<void> fetchPlaylistTracks(String playlistId, String playlistName) async {
        ...
        content: SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final trackCount = playlist['tracks']['total'];
              final playlistId = playlist['id'];
              final isCollaborative = playlist['collaborative'];
...
void showWaitingDialog(BuildContext context, String sessionId, String userName) {
  showDialog(
    context: context,
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                if (users.length > 1) {
                  ...
                  Navigator.of(context).pop();
                  showSessionConnectedScreen(context, sessionId, [widget.userName]);
                }
              }
...
void showSessionConnectedScreen(BuildContext context, String sessionId, List<String> users) {
        ...
        body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 20),
              Text('Connected Users:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ...users.map((user) => Text(user, style: TextStyle(fontSize: 16))),
            ],
          ),
        ),
      ),
          ...
          children: [
            ElevatedButton(
              onPressed: () {
                ...
                createSessionService.createSession(userName, context, (sessionId) {
                  // Handle session creation - You can navigate to the session screen here or update the UI
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => SessionScreen(sessionId: sessionId, userName: userName),
                    ),
                  );
                });
              },
                  ...
                  builder: (context) {
                    return AlertDialog(
                      ...
                      actions: [
                        TextButton(
                          onPressed: () {
                            Navigator.of(context).pop(); // Close the join session dialog
                          },
                          child: const Text('Cancel'),
                        ),
                        TextButton(
                          onPressed: () {
                            ...
                            if (sessionId.isNotEmpty) {
                              ...
                              joinSessionService.joinSession(sessionId, userName, context, () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Please enter a valid session ID')),
                              );
                            }
                          },
...
Future<void> addUserToSession(String sessionId, String userName, BuildContext context) async {
  ...
  if (sessionSnapshot.exists) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Successfully joined session with ID: $sessionId')),
    );
  } else {
    ScaffoldMessenger.of(context).showSnackBar(
...
Future<List<Map<String, dynamic>>> searchSpotifyTracks(String query, String accessToken) async {
  final url = 'https://api.spotify.com/v1/search?q=$query&type=track&limit=10';
  final response = await http.get(
    Uri.parse(url),
    headers: {
...
Future<void> addSongToSession(String sessionId, Map<String, dynamic> track, BuildContext context) async {
  DocumentReference sessionDoc = firestore.collection('sessions').doc(sessionId);
...
void showAddSongDialog(BuildContext context, String sessionId, String accessToken) {
          ...
          children: [
            TextField(
              ...
              onSubmitted: (query) async {
                try {
                  searchResults = await searchSpotifyTracks(query, accessToken);
                  ...
                  return ListTile(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () {
                        ...
                        Navigator.of(context).pop(); // Close dialog after adding song
                      },
                    ),
                  );
                },
